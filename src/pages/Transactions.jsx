...
export default function Transactions() {
  ...
  const filtered = transactions.filter(tx => {
    ...
    if (search) {
      ...
      if (!tx.description?.toLowerCase().includes(q) && !tx.friendName?.toLowerCase().includes(q)) return false;
    }
    return true;
  });
      ...
      <div className="bg-[#556B2F] px-4 pt-12 pb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-[#86EFAC] text-sm mt-1">{filtered.length} records</p>
          </div>
          {filtered.filter(t => t.type === 'expense').length > 0 && (
            <div className="text-right">
              <p className="text-[#86EFAC] text-xs">Total shown</p>
              <p className="text-white font-mono-nums font-bold text-lg">₹{total.toLocaleString('en-IN')}</p>
            </div>
          )}
        </div>
      </div>
        ...
        <div className="flex gap-2 mb-4">
          <div className="flex-1 flex items-center gap-2 bg-white border border-[#E2E8F0] rounded-btn px-3 py-2.5">
            ...
            <input
              ...
              className="flex-1 text-sm outline-none text-[#334155] bg-transparent"
            />
            {search && (
        ...
        {showFilters && (
          <div className="bg-white rounded-card border border-[#E2E8F0] p-4 mb-4">
            <div className="flex items-center justify-between mb-3">
              <p className="text-sm font-semibold text-[#0F172A]">Filters</p>
              {hasFilters && (
                <button onClick={clearFilters} className="text-xs text-red-500 font-medium">Clear all</button>
              )}
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-pill text-xs font-medium border transition-colors ${
                      filterCat === 'all' ? 'bg-[#556B2F] text-white border-[#556B2F]' : 'bg-[#F8F7F2] text-[#4B5563] border-[#E2E8F0]'
                    }`}
                  >All</button>
                  {CATEGORIES.map(c => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-pill text-xs font-medium border transition-colors ${
                        filterCat === c.id ? 'bg-[#556B2F] text-white border-[#556B2F]' : 'bg-[#F8F7F2] text-[#4B5563] border-[#E2E8F0]'
                      }`}
                    >{c.label}</button>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-pill text-xs font-medium border transition-colors ${
                      filterMode === 'all' ? 'bg-[#556B2F] text-white border-[#556B2F]' : 'bg-[#F8F7F2] text-[#4B5563] border-[#E2E8F0]'
                    }`}
                  >All</button>
                  {PAYMENT_MODES.map(m => (
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-pill text-xs font-medium border transition-colors ${
                        filterMode === m.id ? 'bg-[#556B2F] text-white border-[#556B2F]' : 'bg-[#F8F7F2] text-[#4B5563] border-[#E2E8F0]'
                      }`}
                    >{m.label}</button>
                  ))}
                </div>
            ...
            <div className="py-12 text-center">
              ...
              {(search || hasFilters) && (
                <button
                  ...
                  className="mt-2 text-xs text-[#15803D] font-medium"
                >
                  Clear filters
                </button>
              )}
